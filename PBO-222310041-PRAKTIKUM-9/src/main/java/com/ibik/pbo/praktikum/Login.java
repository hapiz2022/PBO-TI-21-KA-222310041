	...
	private void GenerateUI(Login Login) {
		...
		btnSubmit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
                ...
                User users = new User();
                String fullName = login.getFullName(email);
				if (email.isEmpty() || pass.isEmpty()) {
					JOptionPane.showMessageDialog(null, "Harap Masukkan Email Dan Password Anda");
					return;
				}
				if (!email.contains("@")) {
                ...
                try {
                    if (login.authLogin(email, pass)) {
                    	String message = String.format("Selamat Datang, %s!", fullName);
                        ...
                        JOptionPane.showMessageDialog(null, "Email atau Password Anda Salah!", "Alert!", JOptionPane.ERROR_MESSAGE);
                    }
                } catch (Exception err) {
                    JOptionPane.showConfirmDialog(null, "Login Gagal. Error: " + err.getMessage());
                    err.printStackTrace();
                }
